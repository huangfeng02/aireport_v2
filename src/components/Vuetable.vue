          ...
          <template v-if="field.visible">
            <template v-if="isSpecialField(field.name)">
              <th v-if="extractName(field.name) == '__checkbox'" :class="['vuetable-th-checkbox-'+trackBy, field.titleClass]">
                <input type="checkbox" @change="toggleAllCheckboxes(field.name, $event)" :checked="checkCheckboxesState(field.name)"></input>
              </th>
  ...
  <template v-for="(item, index) in tableData">
    <tr @dblclick="onRowDoubleClicked(item, $event)" @click="onRowClicked(item, $event)" :render="onRowChanged(item)" :class="onRowClass(item, index)">
      <template v-for="field in fields">
        <template v-if="field.visible">
          <template v-if="isSpecialField(field.name)">
            <td v-if="extractName(field.name) == '__sequence'" :class="['vuetable-sequence', field.dataClass]" v-html="tablePagination.from + index">
            </td>
          ...
          <template v-else>
            ...
            @click="onCellClicked(item, field, $event)"
            @dblclick="onCellDoubleClicked(item, field, $event)"
            v-html="getObjectValue(item, field.name, '')"></td>
          </template>
      ...
      <template v-if="!noOperation">
        <td style="white-space:nowrap">
          <div>
            <i class="fa fa-fw fa-trash-o" @click="rowDelEvent(item)" title="删除"></i>
            <i class="fa fa-fw fa-wrench" @click="rowEditEvent(item)" title="编辑"></i>
          </div>
        </td>
      </template>
    </tr>
...
<tr v-if="isLoading">
  <td :colspan="fields.length" class="text-center noData">
    <i class="fa fa-spinner fa-spin"></i>
  </td>
</tr>
<tr v-if="!isLoading && !tableData.length">
  <td :colspan="fields.length" class="text-center noData">
    {{emptyTableText}}
  </td>
</tr>
  ...
  export default {
    props: {
      ...
      css: {
        ...
        default: function() {
          return {
            tableClass: 'ui blue selectable celled stackable attached table table-striped',
    ...
    rowDelEvent: function (dataItem) {
      let idColumn = this.trackBy
      let key = dataItem[idColumn]
      this.$emit('delRow', key)
    },
    rowEditEvent: function (dataItem) {
      let idColumn = this.trackBy
      let key = dataItem[idColumn]
      this.$emit('editRow', key)
    }
  }, // end: methods
  watch: {
